...
if submit_button:
    if intern_name and internship_period:
        # Generate the certificate and store it
        cert_id, cert_path = generate_certificate(intern_name, internship_period)
        
        # Save the certificate details in the database
        success = store_certificate_data(cert_id, intern_name, internship_period, cert_path)
        
        if success:
            st.success(f"Certificate generated successfully with ID: {cert_id}")
            
            # Provide download link for the generated certificate
            with open(cert_path, "rb") as file:
                st.download_button(
                    label="Download Certificate",
                    data=file,
            ...
            st.error("Failed to save the certificate.")
    else:
        st.error("Please fill in all the details.")
